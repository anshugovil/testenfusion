...
class DeliverableCalculator:
    ...
    def calculate_deliverables_from_dataframe(self, positions_df: pd.DataFrame, 
        ...
        for idx, row in positions_df.iterrows():
            ...
            spot_price = 0
            if prices:
                # Try underlying first, then symbol, then ticker
                spot_price = prices.get(underlying, prices.get(symbol, prices.get(ticker, 0)))
    ...
    def generate_deliverables_report(self, 
        ...
        if all_positions:
            # Write master deliverables sheet
            self._write_master_sheet(all_positions, prices, "PRE")
            self._write_master_sheet(all_positions, prices, "POST")
            
            # Write expiry-wise deliverable sheets
            expiries = list(set(p['expiry'] for p in all_positions))
            for expiry in sorted(expiries):
                self._write_expiry_sheet(expiry, all_positions, prices, "PRE")
                self._write_expiry_sheet(expiry, all_positions, prices, "POST")
            
            # Write IV sheets
            self._write_iv_master_sheet(all_positions, prices, "PRE")
            self._write_iv_master_sheet(all_positions, prices, "POST")
    ...
    def _write_master_sheet(self, positions: List[Dict], prices: Dict[str, float], prefix: str):
    ...
    def _write_expiry_sheet(self, expiry_date: datetime, positions: List[Dict], 
                           prices: Dict[str, float], prefix: str):
